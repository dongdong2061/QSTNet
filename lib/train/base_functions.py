...
from lib.train.dataset import VisEvent, LasHeR, DepthTrack,RGBT234,LasHeR_testingSet,LasHeR_trainingSet,RGBT210Dataset,GTOTDataset
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val", "COCO17", "VID", "TRACKINGNET",
                        "DepthTrack_train", "DepthTrack_val", "LasHeR_all", "LasHeR_train", "LasHeR_val", "VisEvent","RGBT234","RGBT210","GTOT"]
        ...
        if name == "RGBT234":
            datasets.append(RGBT234(settings.env.rgbt234_dir))
        if name == "RGBT210":
            datasets.append(RGBT210Dataset(settings.env.rgbt210_dir))
        if name == "GTOT":
            datasets.append(GTOTDataset(settings.env.gtot_dir))
        if name == "VisEvent":
...
def get_optimizer_scheduler(net, cfg):
    ...
    if 'bat' in train_type:
        ...
        #     if 'adap' not in n:
        #         # if 'box_head' not in n:  #unfrozen bn
        #         p.requires_grad = False
        #     else:
        #         print(n)
        ...
        for n, p in net.named_parameters():
            ...
            if "adap" not in n:
                if 'box_head' not in n:
                    # if "blocks.11" not in n:
                    #     if "blocks.9" not in n:
                    #         if "blocks.10" not in n:
                                print('冻结的网络结构',n)
                                p.requires_grad = False
                    #         else:
                    #             print(n)
