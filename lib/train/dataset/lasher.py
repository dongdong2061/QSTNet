...
class LasHeR(BaseVideoDataset):
    ...
    def _read_bb_anno(self, seq_path):
        ...
        seq_path = seq_path.replace('trainingset/','')
    ...
    def _get_frame_path(self, seq_path, frame_id):
        # Note original filename is chaotic, we rename them  (now we do not need this) 
        seq_path = seq_path.replace('trainingset/','')
        rgb_frame_path = os.path.join(seq_path, 'visible', sorted([p for p in os.listdir(os.path.join(seq_path, 'visible')) if os.path.
                                                                splitext(p)[1] in ['.jpg','.png','.bmp']])[frame_id])
        
        ir_frame_path = os.path.join(seq_path, 'infrared', sorted([p for p in os.listdir(os.path.join(seq_path, 'infrared')) if os.path.
                                                                splitext(p)[1] in ['.jpg','.png','.bmp']])[frame_id])
        # rgb_frame_path = os.path.join(seq_path, 'visible', '{:0{}d}.jpg'.format(frame_id,num))  # frames start from 0
        # ir_frame_path = os.path.join(seq_path, 'infrared', '{:0{}d}.jpg'.format(frame_id,num))
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        seq_path = self._get_sequence_path(seq_id)
        # seq_path /DATA/dingzhaodong/project/BAT/data/lasher/trainingset/1strowrightgirl frame_ids [3342]
        # print('seq_path',seq_path,'frame_ids',frame_ids)
