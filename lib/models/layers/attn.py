...
class Attention_Uncertainty(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 rpe=False, z_size=7, x_size=14):
    ...
    def forward(self, x, mask=None, return_attention=False):
        ...
        # 在列的维度（dim=1）上求和，并保持该维度 构建主观逻辑意见 多头注意力机制 #
        evidence = torch.sum(attn_e, dim=2, keepdim=True).permute(0,1,3,2).squeeze(-1)
        alpha = self.Softplus(evidence) + 1
        S = torch.sum(alpha,dim=-1).unsqueeze(-1)
        b = evidence / S
        u = N / S
        # 计算每个batch的最小值和最大值
        min_u = u.min(dim=1, keepdim=True).values
        max_u = u.max(dim=1, keepdim=True).values
        # 计算归一化后的权重
        weights_u = 0.01 + (u - min_u) / (max_u - min_u+0.0001) * (1 - 0.01)
        weights_u =  1 - weights_u
        x = (attn @ v *weights_u.unsqueeze(-1)).transpose(1, 2).reshape(B, N, C)
        # sum_u = u.sum(dim=1, keepdim=True)
        # weights_heads = u / sum_u
        #----------------------------------------------# 
