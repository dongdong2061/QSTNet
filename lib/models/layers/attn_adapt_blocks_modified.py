...
from lib.models.layers.adapter import Bi_direct_adapter,Bi_appearence_adapter,Bi_cross_adapter
...
def candidate_elimination(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    ...
    # print('tokens_new',tokens_new.size(),'keep_index',keep_index.size(),'removed_index',removed_index.size())
...
class CEABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        # self.adapter_rgb = Bi_cross_adapter(type='RGB')
        # self.adapter_tir = Bi_cross_adapter()
        # self.adapter_rgb2 = Bi_cross_adapter(type='RGB')
        # self.adapter_tir2 = Bi_cross_adapter()

        self.adapter_1 = Bi_appearence_adapter()
        self.adapter_2 = Bi_appearence_adapter()
    ...
    def forward(self, x, xi, global_index_template, global_index_templatei, global_index_search, global_index_searchi, mask=None, ce_template_mask=None, keep_ratio_search=None):
        #torch.Size([32, 320, 768]) torch.Size([32, 320, 768]) torch.Size([32, 64]) torch.Size([32, 64]) torch.Size([32, 256]) torch.Size([32, 256])
        # print(x.size(),xi.size(),global_index_template.size(),global_index_templatei.size(),global_index_search.size(),global_index_searchi.size())
        xori = x
        x_norm1 = self.norm1(x)
        xi_norm1 = self.norm1(xi)

        
        #x_attn torch.Size([32, 320, 768]) attn torch.Size([32, 12, 320, 320])
        x_attn, attn = self.attn(self.norm1(x), mask, True)
        # print("x_attn",x_attn.shape,"attn",attn.shape)   
        
        #x torch.Size([32, 320, 768])
        # x = x + self.drop_path(x_attn) + self.drop_path(self.adap_t(self.norm1(xi)))  #########-------------------------adapter
        # print('x',x.shape)
        x_appearance = self.drop_path(self.adapter_1(x_norm1,xi_norm1))
        x = x + self.drop_path(x_attn) + x_appearance
        ...
        xi = xi + self.drop_path(xi_attn) + x_appearance            
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            ...
            # # 计算差值
            # difference = global_index_search - global_index_searchi
            # # 计算逐元素的绝对差值
            # absolute_difference = torch.abs(difference)
            # # print(absolute_difference.size())
            # # 计算所有元素的均值
            # mean_difference = torch.mean(absolute_difference.float()).item()
            # # print(mean_difference,mean_difference.size())
            # diff = mean_difference / 100
            # print("diff",diff)

        xori = x
        x_norm2 = self.norm2(x)
        xi_norm2 = self.norm2(xi)
        x_appearance = self.drop_path(self.adapter_2(x_norm2,xi_norm2))
   
        x = x + self.drop_path(self.mlp(self.norm2(x))) + x_appearance
        xi = xi + self.drop_path(self.mlp(self.norm2(xi))) + x_appearance
        ...
        #torch.Size([32, 320, 768]) torch.Size([32, 320, 768])
        # print(x.size(),xi.size())
