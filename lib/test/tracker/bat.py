...
class BATTrack(BaseTracker):
    ...
    def track(self, image, info: dict = None,vis_feat=False):
        ...
        ###---for temporal information 0810, for memory bank---###
        # if self.last_template is None:
        #     self.last_template = self.z_tensor
        #     self.z_tensor_ = torch.cat([self.initial_template,self.last_template],dim=0)
        #     #for reduce error
        #     self.templates_list.append(self.last_template)
        #     self.templates_list.append(self.last_template)
        # else:
        #     #for reduce error
        #     if len(self.templates_list) == 5:
        #         self.templates_list.pop(0)
        #     else:
        #         self.last_template = self.templates_list[0]
        #         self.templates_list.append(new_template)
        #     self.last_template = self.last_template
        #     self.z_tensor_ = torch.cat([self.initial_template,new_template],dim=0)
        #No Memoery bank
        if self.last_template is None:
            self.last_template = self.z_tensor
            self.z_tensor_ = torch.cat([self.initial_template,self.last_template],dim=0)
            ...
            #     self.last_template = self.templates_list[0]
            #     self.templates_list.append(new_template)
            self.last_template = new_template
            self.z_tensor_ = torch.cat([self.initial_template,new_template],dim=0)
        ###-------###
        ...
        with torch.no_grad():
            # x_tensor = search
            #将模板和搜索区域都设置为两帧
            x_tensor = torch.cat([search,search],dim=0)
            # merge the template and the search
            # run the transformer
            out_dict = self.network.forward(
                template=self.z_tensor_, search=x_tensor, ce_template_mask=self.box_mask_z,Test=True,frame_id=self.frame_id)
