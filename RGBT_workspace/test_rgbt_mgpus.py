...
def genConfig(seq_path, set_type):
    if set_type == 'RGBT234':
        ...
        #图片格式为jpg
        ############################################  have to refine #############################################
        RGB_img_list = sorted([seq_path + '/v/' + p for p in os.listdir(seq_path + '/v') if os.path.splitext(p)[1] == '.jpg'])
        T_img_list = sorted([seq_path + '/i/' + p for p in os.listdir(seq_path + '/i') if os.path.splitext(p)[1] == '.jpg'])
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter'):
    ...
    save_path = f'./RGBT_workspace/results/{dataset_name}/' + save_name +'15'+  '/' + seq_txt + '.txt'
    save_folder = f'./RGBT_workspace/results/{dataset_name}/' + save_name+'15'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default=None, help='Name of dataset (GTOT,RGBT234,LasHeR,VTUAVST,VTUAVLT).')
    ...
    if dataset_name == 'GTOT':
        ...
        # seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
        # /DATA/dingzhaodong/project/BAT2/BAT/RGBT_workspace/test_rgbt_mgpus.py
